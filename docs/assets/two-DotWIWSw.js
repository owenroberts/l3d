import{c as it,r as f,d as st,e as se,m as te,D as xt}from"./Doodoo-CtZp_IRM.js";import{C as ye,S as at,M as G,I as he,a as le,O as W,V as x,b as rt,D as Ce,c as ct,d as bt,L as Ee,T as Re,U as lt,e as ke,f as Ae,B as je,g as St,F as Xe,h as Te,i as pe,W as Mt,j as dt,k as Ue,l as Pt,m as kt,n as At,o as _t,P as Lt,p as zt,A as Ct,q as Et,r as ut,s as Rt,t as Ut,u as Bt}from"./three-Bp_Ykcir.js";import{R as qe,L as Ze,E as Ye,A as V,O as Dt,S as Tt,v as Ot,b as Ft,m as pt,J,a as jt,c as It}from"./blend-D_kk9L8V.js";function Gt(c){const{scene1:e,scene2:o,renderer:t,camera:n,noScene2:w}=c,u=new qe(e,n),i=new Ze({width:t.domElement.clientWidth,height:t.domElement.clientHeight,scene:e,camera:n,uniforms:{lineColor:{type:"vec3",value:new ye(0)},bgColor:{type:"vec3",value:new ye(13092807)},lineWidth:1,numLines:5,diffuseCutoff:{type:"float",value:40},normalCutoff:{type:"float",value:50},noiseMultiplier:{type:"float",value:10}}}),r=new Ye(t);r.addPass(u),r.addPass(i);let a,d;const h={diffuse:new V({value:40,valueClamp:[30,60],increment:1,randomRange:[-1,1],clampRange:[-3,2],count:24}),normal:new V({value:50,valueClamp:[10,100],increment:1,randomRange:[-1,1],clampRange:[-3,2],count:24}),noise:new V({value:10,valueClamp:[2,14],increment:1,randomRange:[-1,1],clampRange:[-3,2],count:24})};if(!w){const m=new qe(o,n);d=new Ze({width:t.domElement.clientWidth,height:t.domElement.clientHeight,scene:o,camera:n,uniforms:{lineColor:{type:"vec3",value:new ye(16777215)},bgColor:{type:"vec3",value:new ye(13092807)},lineWidth:1,numLines:5,diffuseCutoff:{type:"float",value:40},normalCutoff:{type:"float",value:50},noiseMultiplier:{type:"float",value:10}}});const p=new Dt;a=new Ye(t),a.renderToScreen=!1,a.addPass(m),a.addPass(d),a.addPass(p);const _=new Tt(new at({uniforms:{baseTexture:{value:null},blendTexture:{value:a.renderTarget2.texture}},vertexShader:Ot,fragmentShader:Ft,defines:{}}),"baseTexture");_.needsSwap=!0,r.addPass(_)}function l(m){i.material.uniforms.diffuseCutoff.value=h.diffuse.update(),i.material.uniforms.normalCutoff.value=h.normal.update(),i.material.uniforms.noiseMultiplier.value=h.noise.update()}function g(){w||a.render(),r.render()}function s(m,p){r.setSize(m,p),a&&a.setSize(m,p)}return{process:g,update:l,setSize:s}}function Wt(c){const{worldRadius:e,scene:o}=c,t=new G(new he(e,3),new le({color:65535}));t.castShadow=!1,t.receiveShadow=!0;const n=pt(t.geometry),w=n.getAttribute("position"),u=n.getAttribute("normal"),i=e*4/Math.sqrt(10+2*Math.sqrt(5));function r(l){const g=a(l),s=it(g),m=g.filter(_=>_.index!=s.index).map(_=>_.index),p=f(a(s.position,m));if(p&&st(.5)){const _=new W;_.position.copy(s.position),_.up.copy(s.normal),_.lookAt(p.position),_.translateZ(s.position.distanceTo(p.position)/2);const z=new x().copy(s.normal).add(p.normal).divideScalar(2);return{position:_.position,normal:z}}return s}function a(l,g){const s=[];g||(g=Array.from(Array(w.count).keys()));const m=new x;for(let p=0;p<g.length;p++){const _=g[p];m.fromBufferAttribute(w,_);const z=m.distanceTo(l);z>0&&z<i/3&&s.push(d(_))}return s}function d(l){return{position:new x().fromBufferAttribute(w,l),normal:new x().fromBufferAttribute(u,l),index:l}}function h(){return se(w.count-1)}return{getNext:r,getGlobePos:d,getRandomVertex:h,getGlobe:()=>t}}function Nt(){let c=0,e=new rt;function o(){c===6&&(e.x=(e.x+f(-.1,.1)).clamp(0,2),e.y=(e.y+f(-.1,.1)).clamp(0,2),c=0),c++}return{update:o,getValue:()=>e}}function Vt(c){const{camera:e}=c;let o=new x,t=new W;function n(){o.setFromMatrixPosition(t.matrixWorld),e.position.lerp(o,.02)}return{update:n,getGoal:()=>t}}function Ht(c){const{globe:e,scene:o}=c;let t,n,w=1e6;const u=new W,i=new le({color:4013373,side:Ce});function r(){const y=e.getRandomVertex();t=e.getGlobePos(y),n=e.getNext(t.position),u.position.set(t.position.x,t.position.y,t.position.z),u.up.copy(t.normal),u.lookAt(n.position)}let a=.005;const d=.5,h=new J,l=new J,g=[],s=[],m=.6,p=.3,_=d*2.2,z=d*5,v=3,A=4,M=7;function b(){const y=new ct(d,d*2,2,5),L=new G(y,i);L.castShadow=!0,h.add(L),h.setPosition(0,_,0),h.rotateX(Math.PI*.5),h.setOrigins(),h.setLerpSpeed(2),u.add(h.get());const I=new he(d*1.5,1),U=new G(I,i);U.castShadow=!0,l.add(U),l.setPosition(0,z,d*2),l.randomRotation(),l.setOrigins(),u.add(l.get());const R=new bt(d*1,d*1,3),me=new G(R,i);me.castShadow=!0;const ue=new J;ue.add(me),ue.addPosition(d*-1,d*1.5,0),ue.randomRotation(),l.add(ue.get());const ve=new J;ve.add(me.clone()),ve.addPosition(d*1,d*1.5,0),ve.randomRotation(),l.add(ve.get());for(let N=0;N<M;N++){const E=new J,K=d*.6;N===0?(E.copy(h.get().position),E.addPosition(0,d*1.2,-d*1.2),E.rotateX(-Math.PI*.2),u.add(E.get())):(E.addPosition(0,K,0),g[N-1].add(E)),E.rotateX(N*Math.PI*.02),E.add(P(new x(0,0,0),new x(0,K,0))),E.setOrigins(),E.setRotateSpeed(v),g.push(E)}for(let N=0;N<4;N++){const E=new J,K=new J,We=d*1,Ne=.2,Ve=new x(0,0,0),He=new x(0,-We,0);E.copy(h.getPosition()),E.addPosition(0,d*-.5,0),E.rotateX(Math.PI*Ne),E.setOrigins(),E.setRotateSpeed(A),E.add(P(Ve,He)),K.setPosition(0,-We,0),K.rotateX(Math.PI*-Ne*2),K.add(P(Ve,He)),K.setRotateSpeed(A),u.add(E.get()),E.add(K),s.push({joints:[E,K],phase:N*2})}const ee=d*.6;s[0].joints[0].addPosition(ee,0,ee),s[2].joints[0].addPosition(ee,0,-ee),s[1].joints[0].addPosition(-ee,0,ee),s[3].joints[0].addPosition(-ee,0,-ee),s[0].joints[0].rotateY(m),s[1].joints[0].rotateY(-m),s[2].joints[0].rotateY(-p),s[3].joints[0].rotateY(p),s.forEach(N=>N.joints.forEach(E=>E.setOrigins()))}b();function P(y,L){const I=new Ee(y,L),U=new Re(I,1,.08,3),R=new G(U,i);return R.castShadow=!0,u.add(R),R}function S(){const y=new he(f(.01,.05),1),L=new G(y,i);L.position.copy(u.position),L.quaternion.copy(u.quaternion),L.translateX(f(-.8,.8)),L.translateZ(f(1)),o.add(L)}const k={walk:{tail:new V({increment:3,randomRange:[-.1,.1],clampRange:[-.5,.5],func:y=>te(Math.sin(y),-1,1,.2,.3)}),legs:new V({increment:4,randomRange:[-.1,.1],clampRange:[-.2,.2]}),body:new V({increment:10,func:y=>te(Math.sin(y),-1,1,0,.5)})},idle:{head:new V({increment:1.25,randomRange:[-.1,.1],clampRange:[-.25,0],func:y=>te(Math.sin(y+.1),-1,1,-1,1)}),tail:new V({increment:1.25,randomRange:[-.1,.1],clampRange:[-.25,0],func:y=>te(Math.sin(y),-1,1,-1.2,1.2)})},crumbs:new V({increment:1,count:36,randomRange:[-1,1],clampRange:[-10,10],func:(y,L)=>{L.isCount&&S()}})};function C(y){const L=k.walk.tail.update(y);for(let R=1;R<g.length;R++)g[R].setTargetRotation({x:L+R*.05}),g[R].rotate(y);const I=k.walk.legs.update(y);for(let R=0;R<s.length;R++){const me=te(Math.sin(s[R].phase+I),-1,1,-2,2),ue=te(Math.sin(s[R].phase+I*2),-1,1,1.25,-1.25);s[R].joints[0].setTargetRotation({x:me}),s[R].joints[0].rotate(y),s[R].joints[1].setTargetRotation({x:ue}),s[R].joints[1].rotate(y)}const U=k.walk.body.update(y);h.setTargetPosition({y:_+U}),h.lerp(y),l.setTargetPosition({y:z-U}),l.lerp(y),k.crumbs.update(y)}function O(y){for(let L=1;L<g.length;L++)g[L].unrotate(y);for(let L=0;L<s.length;L++)s[L].joints[0].unrotate(y),s[L].joints[1].unrotate(y);h.unlerp(y)}function F(y){const L=k.idle.head.update(y);l.setTargetRotation({y:L}),l.rotate(y);const I=k.idle.tail.update(y);for(let U=1;U<g.length;U++)g[U].setTargetRotation({z:I+U*.05*Math.sign(I)}),g[U].rotate(y)}function j(y,L){if(isNaN(y))return;let I=y/1e3;if(L){C(I);const U=u.position.distanceTo(n.position);U>.1&&w-U>0?(u.translateZ(a*y),w=U):(w=1e6,u.up.copy(n.normal),n=e.getNext(n.position),u.lookAt(n.position))}else h.isAtOrigin()?F(I):O(I)}function de(y){y.code}return document.addEventListener("keydown",de),{update:j,globeSetup:r,getStart:()=>t,isLoaded:()=>!0,getModel:()=>u}}ke.line={worldUnits:{value:1},linewidth:{value:1},resolution:{value:new rt(1,1)},dashOffset:{value:0},dashScale:{value:1},dashSize:{value:1},gapSize:{value:1}};Ae.line={uniforms:lt.merge([ke.common,ke.fog,ke.line]),vertexShader:`
		#include <common>
